...
{
	public abstract class AbstractCompositeSerializer<TValue, T1, T2, T3, T4, T5, T6, T7> :
		ISerializerDeserializer<TValue>
	{
		...
		private readonly ISerializerDeserializer<T7> serialzierT7;
		...
		public AbstractCompositeSerializer(
			...
			ISerializerDeserializer<T7> serialzierT7)
		{
			...
			Contracts.Requires.That(EndianUtilities.AllSameEndianness(
				serialzierT1, serialzierT2, serialzierT3, serialzierT4, serialzierT5, serialzierT6, serialzierT7));
			...
			this.serialzierT7 = serialzierT7;
		}
		...
		public virtual int GetSerializedLength(TValue value)
		{
			...
			T6 part6;
			T7 part7;
			this.DecomposeValue(value, out part1, out part2, out part3, out part4, out part5, out part6, out part7);
		...
		public TValue Deserialize(IBufferedArray buffer)
		{
			...
			T7 part7 = this.serialzierT7.Deserialize(buffer);

			return this.ComposeValue(part1, part2, part3, part4, part5, part6, part7);
		}
		...
		public TValue Deserialize(byte[] buffer, ref int index)
		{
			...
			T7 part7 = this.serialzierT7.Deserialize(buffer, ref index);

			return this.ComposeValue(part1, part2, part3, part4, part5, part6, part7);
		}

		protected abstract TValue ComposeValue(T1 part1, T2 part2, T3 part3, T4 part4, T5 part5, T6 part6, T7 part7);
