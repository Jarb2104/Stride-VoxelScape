...
{
	public abstract class AbstractCompositeSerializer<TValue, T1, T2, T3, T4, T5> :
		ISerializerDeserializer<TValue>
	{
			...
			ISerializerDeserializer<T5> serialzierT5)
		{
			...
			Contracts.Requires.That(EndianUtilities.AllSameEndianness(
				serialzierT1, serialzierT2, serialzierT3, serialzierT4, serialzierT5));
		...
		protected abstract TValue ComposeValue(T1 part1, T2 part2, T3 part3, T4 part4, T5 part5);

		protected abstract void DecomposeValue(
			TValue value, out T1 part1, out T2 part2, out T3 part3, out T4 part4, out T5 part5);
	}
}
