...
namespace Voxelscape.Common.Indexing.Core.Arrays
{
	...
	public class DynamicArray3D<T> : AbstractDynamicallyBoundedIndexable3D<T>
	{
		/// <summary>
		...
		private void HandleArrayResizing(Index3D index)
		{
			...
			int xNewSize = DynamicArrayUtilities.HandleAxis(this.GetCurrentLength(Axis3D.X), index.X);
			int yNewSize = DynamicArrayUtilities.HandleAxis(this.GetCurrentLength(Axis3D.Y), index.Y);
			int zNewSize = DynamicArrayUtilities.HandleAxis(this.GetCurrentLength(Axis3D.Z), index.Z);

			// copy old array into new array
			T[,,] newArray = new T[xNewSize, yNewSize, zNewSize];

			foreach (KeyValuePair<Index3D, T> entry in this)
			{
				newArray[entry.Key.X, entry.Key.Y, entry.Key.Z] =
					this.array[entry.Key.X, entry.Key.Y, entry.Key.Z];
			}
