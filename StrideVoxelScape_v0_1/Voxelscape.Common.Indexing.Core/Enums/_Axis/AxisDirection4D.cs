namespace Voxelscape.Common.Indexing.Core.Enums
{
	/// <summary>
	/// An enumeration of the orthogonal axes in four dimensional space
	/// used to specify a single axis at a time and a direction along that axis.
	/// </summary>
	public enum AxisDirection4D
	{
		/// <summary>
		/// The positive direction along the x axis.
		/// </summary>
		PositiveX,

		/// <summary>
		/// The negative direction along the x axis.
		/// </summary>
		NegativeX,

		/// <summary>
		/// The positive direction along the y axis.
		/// </summary>
		PositiveY,

		/// <summary>
		/// The negative direction along the y axis.
		/// </summary>
		NegativeY,

		/// <summary>
		/// The positive direction along the z axis.
		/// </summary>
		PositiveZ,

		/// <summary>
		/// The negative direction along the z axis.
		/// </summary>
		NegativeZ,

		/// <summary>
		/// The positive direction along the w axis.
		/// </summary>
		PositiveW,

		/// <summary>
		/// The negative direction along the w axis.
		/// </summary>
		NegativeW,
	}
}
