...
{
	/// <summary>
	/// An indexable array made up of a 3 by 3 grid of identically sized indexable arrays.
	/// </summary>
	/// <remarks>
	...
	internal class Composite3By3Array3D<T> : AbstractBoundedIndexable3D<T>
	{
		/// <summary>
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Composite3By3Array3D{TValue}"/> class.
		/// </summary>
		/// <param name="arrays">The indexable arrays to make the composite indexable out of.</param>
		public Composite3By3Array3D(IBoundedIndexable<Index3D, T>[,] arrays)
		{
			...
			this.arrays = arrays;
			IBoundedIndexable<Index3D, T> array = this.arrays[1, 1];

			this.singleArrayLengthX = array.GetLength(Axis3D.X);
			this.singleArrayLengthZ = array.GetLength(Axis3D.Z);
			Index3D indexingOffset = new Index3D(this.singleArrayLengthX, 0, this.singleArrayLengthZ);
		...
		private IBoundedIndexable<Index3D, T> GetIndexable(int x, int z)
		{
